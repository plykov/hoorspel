...
function requestHost(req: { headers: Record<string, string | string[] | undefined> }): string {
  const forwarded = req.headers["x-forwarded-host"];
  const host = forwarded ?? req.headers.host ?? req.headers[":authority"];
  return Array.isArray(host) ? (host[0] ?? "") : (host ?? "");
}

/** Merge share_target into the platform manifest; land POST /import on ?shared=1. */
function shareTargetPlugin(): Plugin {
  function attach(middlewares: {
    ...
    middlewares.use((req, res, next) => {
      ...
      if (method === "POST" && pathOnly === "/import") {
        ...
        req.on("end", () => {
          res.statusCode = 303;
          res.setHeader("location", "/import?shared=1");
          res.end();
        });
        return;
      }
      ...
      if (
        method === "GET" &&
        (pathOnly === "/__grok/manifest.webmanifest" || pathOnly === "/__grok/manifest.json")
      ) {
        const manifest = JSON.parse(renderWebManifest(requestHost(req))) as Record<string, unknown>;
...
export default defineConfig(({ command, isPreview }) => ({
  ...
  plugins: [
    ...
    appEnvPlugin(),
    // Share-target merge + POST /import redirect — before grokPwa so we win the
    // manifest path without editing platform files.
    shareTargetPlugin(),
